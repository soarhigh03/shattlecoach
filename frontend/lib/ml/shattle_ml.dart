...
const List<String> kShattleStrokes = <String>['high_clear', 'short_serve', 'forehand_drive'];
...
class ShattleScore {
  ...
  final List<String> axesEvaluated; // which of posture/speed/step apply to this stroke
...
class ShattleMl {
  ...
  static Future<ShattleScore> analyzeSwing({
    ...
    if (ext.detectionRate < 0.5) {
      sw.stop();
      final String msg = '영상에서 사람을 잘 인식하지 못했어요 (${(ext.detectionRate * 100).toStringAsFixed(0)}%). '
          '옆에서 전신이 한 화면에 나오게 다시 촬영해 주세요.';
      return ShattleScore(
        status: 'REJECTED_LOW_POSE_DETECTION', messageKo: msg,
        stroke: stroke, axesEvaluated: const <String>[],
        positives: const <String>[], coachingParagraph: '',
        processingSeconds: sw.elapsedMilliseconds / 1000.0,
        raw: <String, dynamic>{'status': 'REJECTED_LOW_POSE_DETECTION', 'message_ko': msg},
      );
    }

    onStatus?.call('폼 채점 중…');
    final List<List<List<double>>> kpts =
        ext.frames.map((PoseFrame f) => f.keypoints).toList();
    ...
    final List<CriterionResult> criteria = _calib!.apply(
        rule.criteria, stroke, outSource: (String s) => calibSource = s);
    final List<double> validProbs = criteria
        .where((CriterionResult c) => c.measurement != null)
        .map((CriterionResult c) => c.probability).toList();
    final int? postureStars = validProbs.isEmpty
        ? null
        : (1 + (4 * (validProbs.reduce((double a, double b) => a + b) / validProbs.length))).round();
    ...
    final Map<String, dynamic> payload = <String, dynamic>{
      ...
      'posture_criteria': criteria.map((CriterionResult c) => c.toJson()).toList(),
      'speed_details': speed.details,
      'step_details': step.details,
      'pose_detection_rate': ext.detectionRate,
      'pose_extraction_seconds': ext.extractSeconds,
    };
    _filter!.apply(payload); // keys off the selected stroke; drops irrelevant axes' keys
    ...
    final List<String> axes = ((payload['axes_evaluated'] as List<dynamic>?) ?? const <dynamic>[])
        .map((dynamic e) => e.toString()).toList();
    final List<String> positives = ((rb['positives'] as List<dynamic>?) ?? const <dynamic>[])
        .map((dynamic e) => (e as Map<String, dynamic>)['text']?.toString() ?? '')
