...
PercentileTable _fromJson(Map<String, dynamic> j) {
  return PercentileTable(
    (j['quantiles_pct'] as List<dynamic>)
        .map<double>((dynamic v) => (v as num).toDouble())
        .toList(),
    (j['quantiles_value'] as List<dynamic>)
        .map<double>((dynamic v) => (v as num).toDouble())
        .toList(),
  );
}
...
class BstPercentiles {
  ...
  static Future<BstPercentiles> load() async {
    final Map<String, dynamic> s =
        jsonDecode(
              await rootBundle.loadString('assets/bst_speed_percentiles.json'),
            )
            as Map<String, dynamic>;
    final Map<String, dynamic> k =
        jsonDecode(
              await rootBundle.loadString('assets/bst_step_percentiles.json'),
            )
            as Map<String, dynamic>;
...
class _SpeedFeatures {
  ...
  _SpeedFeatures(
    this.maxVelocityNorm,
    this.meanVelocityAtImpact,
    this.impactFrame,
  );
}
...
String _dominantWrist(List<List<List<double>>> kpts) {
  ...
  for (int i = 1; i < kpts.length; i++) {
    final double rdx =
        kpts[i][_idx['right_wrist']!][0] - kpts[i - 1][_idx['right_wrist']!][0];
    final double rdy =
        kpts[i][_idx['right_wrist']!][1] - kpts[i - 1][_idx['right_wrist']!][1];
    rDisp += math.sqrt(rdx * rdx + rdy * rdy);
    final double ldx =
        kpts[i][_idx['left_wrist']!][0] - kpts[i - 1][_idx['left_wrist']!][0];
    final double ldy =
        kpts[i][_idx['left_wrist']!][1] - kpts[i - 1][_idx['left_wrist']!][1];
...
double _bodyScale(List<List<List<double>>> kpts) {
  ...
  for (int i = 0; i < kpts.length; i++) {
    final double shX =
        (kpts[i][_idx['left_shoulder']!][0] +
            kpts[i][_idx['right_shoulder']!][0]) /
        2;
    final double shY =
        (kpts[i][_idx['left_shoulder']!][1] +
            kpts[i][_idx['right_shoulder']!][1]) /
        2;
    final double hpX =
        (kpts[i][_idx['left_hip']!][0] + kpts[i][_idx['right_hip']!][0]) / 2;
    final double hpY =
        (kpts[i][_idx['left_hip']!][1] + kpts[i][_idx['right_hip']!][1]) / 2;
...
AxisScoreResult scoreSpeed(
  List<List<List<double>>> kpts,
  PercentileTable speedTable,
) {
...
AxisScoreResult scoreStep(
  List<List<List<double>>> kpts,
  PercentileTable stepTable,
) {
  if (kpts.length < 2) {
    return AxisScoreResult(
      star: 1,
  ...
  final double body = math.max(_bodyScale(kpts), 1e-6);
  final List<double> laS = <double>[];
  final List<double> raS = <double>[];
  for (int i = 1; i < kpts.length; i++) {
    final double ldx =
        kpts[i][_idx['left_ankle']!][0] - kpts[i - 1][_idx['left_ankle']!][0];
    final double ldy =
        kpts[i][_idx['left_ankle']!][1] - kpts[i - 1][_idx['left_ankle']!][1];
    laS.add(math.sqrt(ldx * ldx + ldy * ldy) / body);
    final double rdx =
        kpts[i][_idx['right_ankle']!][0] - kpts[i - 1][_idx['right_ankle']!][0];
    final double rdy =
        kpts[i][_idx['right_ankle']!][1] - kpts[i - 1][_idx['right_ankle']!][1];
  ...
  double minMin = double.infinity;
  int impact = 0;
  for (int i = 0; i < kpts.length; i++) {
    final double m = math.min(
      kpts[i][_idx['right_wrist']!][1],
      kpts[i][_idx['left_wrist']!][1],
    );
    if (m < minMin) {
      minMin = m;
      impact = i;
  ...
  final double hipPrep =
      (kpts[prep][_idx['left_hip']!][0] + kpts[prep][_idx['right_hip']!][0]) /
      2;
  final double hipImp =
      (kpts[impact][_idx['left_hip']!][0] +
          kpts[impact][_idx['right_hip']!][0]) /
      2;
