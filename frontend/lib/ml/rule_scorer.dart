...
class RuleScoreResult {
  ...
  RuleScoreResult({
    ...
    required this.prepFrame,
    required this.dominantArm,
...
RuleScoreResult scoreRules(
  List<List<List<double>>> kpts, {
  int smoothWindow = 5,
  double minJointConf = 0.3,
}) {
  ...
  final List<int> need1 = <int>[
    _idx['${dom}_shoulder']!,
    _idx['${dom}_wrist']!,
  ];
  if (!_jointsVisible(sm, impact, need1, minJointConf)) {
    results.add(
      na(kCriteriaNames[0], 'ratio_of_shoulder_hip', 0.0, 'pose_conf_too_low'),
    );
  } else {
  ...
  if (!_jointsVisible(sm, impact, need2, minJointConf)) {
    ...
    final double prob = _sigmoidMargin(
      elbowAngle,
      160.0,
      scale: 8.0,
      direction: '>=',
...
List<List<List<double>>> _temporalMedian(
  List<List<List<double>>> kpts,
  int window,
) {
  ...
  for (int ti = 0; ti < t; ti++) {
    ...
    for (int j = 0; j < 17; j++) {
      ...
      final List<double> xs = <double>[
        for (int k = lo; k < hi; k++) kpts[k][j][0],
      ];
      final List<double> ys = <double>[
        for (int k = lo; k < hi; k++) kpts[k][j][1],
      ];
      xs.sort();
      ys.sort();
...
double _angleDeg(List<double> a, List<double> b, List<double> c) {
  final double bax = a[0] - b[0];
  final double bay = a[1] - b[1];
  final double bcx = c[0] - b[0];
  final double bcy = c[1] - b[1];
...
double _hipAngle(List<List<List<double>>> kpts, int t) {
  final double vx =
      kpts[t][_idx['right_hip']!][0] - kpts[t][_idx['left_hip']!][0];
  final double vy =
      kpts[t][_idx['right_hip']!][1] - kpts[t][_idx['left_hip']!][1];
  return math.atan2(vy, vx) * 180.0 / math.pi;
}

double _sigmoidMargin(
  double measurement,
  double threshold, {
