...
class RulebookSentence {
  ...
  final String severity;   // 'pass' | 'minor_fail' | 'major_fail' | 'na' | 'starN'
  final String text;
  RulebookSentence(this.axis, this.criterion, this.severity, this.text);
  Map<String, dynamic> toJson() => <String, dynamic>{
    'axis': axis, 'criterion': criterion, 'severity': severity, 'text': text,
  };
}
...
class RulebookTips {
  ...
  String get naDefault =>
      (_cfg['na_default'] as String?) ?? '이 항목은 측정하지 못했어요.';
  ...
  List<String> forbiddenKeywords(String stroke) {
    final Map<String, dynamic> g = (_cfg['guardrails'] as Map<String, dynamic>?) ?? <String, dynamic>{};
    final Map<String, dynamic>? entry = g[stroke] as Map<String, dynamic>?;
    final List<dynamic> kw = (entry?['forbidden_keywords'] as List<dynamic>?) ?? const <dynamic>[];
  ...
  Map<String, dynamic> _strokeEntry(String stroke) {
    final Map<String, dynamic> strokes = (_cfg['strokes'] as Map<String, dynamic>?) ?? <String, dynamic>{};
  ...
  String? _postureText(String stroke, String criterion, String severity, [int idx = 0]) {
    final Map<String, dynamic> entry = _strokeEntry(stroke);
    final Map<String, dynamic>? posture = entry['posture'] as Map<String, dynamic>?;
    final Map<String, dynamic>? crit = posture?[criterion] as Map<String, dynamic>?;
    final List<dynamic>? bucket = crit?[severity] as List<dynamic>?;
    if (bucket != null && bucket.isNotEmpty) return bucket[idx % bucket.length].toString();
    return null;
  }
  ...
  List<RulebookSentence> selectSentences(Map<String, dynamic> payload, {int seed = 0}) {
    final String stroke = ((payload['stroke'] as Map<String, dynamic>?)?['label']?.toString()) ?? 'unknown_ood';
    final List<String> axesEval = ((payload['axes_evaluated'] as List<dynamic>?) ?? const <dynamic>['posture', 'speed', 'step'])
    ...
    if (axesEval.contains('posture')) {
      ...
      for (final dynamic c in (payload['posture_criteria'] as List<dynamic>?) ?? const <dynamic>[]) {
      ...
      for (int pos = 0; pos < kCriteriaOrder.length; pos++) {
        ...
        final int idx = seed + pos; // per-criterion offset; one variant per cell
        final String? text =
            _postureText(stroke, name, sev, idx) ?? _postureText(stroke, name, 'major_fail', idx);
    ...
    for (final List<String> axisKey in <List<String>>[
      ...
      if (star == null || star >= 3) continue; // corrections only for 1-2; 4-5 → positives
      final String? text = _axisText(stroke, axis, star, seed + star);
      if (text != null) out.add(RulebookSentence(axis, axis, 'star$star', text));
    }
  ...
  List<RulebookSentence> selectPositives(Map<String, dynamic> payload, {int seed = 0}) {
    final String stroke = ((payload['stroke'] as Map<String, dynamic>?)?['label']?.toString()) ?? 'unknown_ood';
    final List<String> axesEval = ((payload['axes_evaluated'] as List<dynamic>?) ?? const <dynamic>['posture', 'speed', 'step'])
        .map((dynamic e) => e.toString()).toList();
    final List<RulebookSentence> out = <RulebookSentence>[];
    if (axesEval.contains('posture')) {
      ...
      for (int pos = 0; pos < kCriteriaOrder.length; pos++) {
        ...
        if (text != null) out.add(RulebookSentence('posture', name, 'pass', text));
      }
    }
    for (final List<String> axisKey in <List<String>>[
      <String>['speed', 'speed_stars'], <String>['step', 'step_stars'],
    ]) {
      final String axis = axisKey[0];
      if (!axesEval.contains(axis)) continue;
      final int? star = (payload[axisKey[1]] as num?)?.toInt();
...
Map<String, dynamic> composeCoaching(RulebookTips bank, Map<String, dynamic> payload, {int seed = 0}) {
  final List<RulebookSentence> sentences = bank.selectSentences(payload, seed: seed);
  final List<RulebookSentence> positives = bank.selectPositives(payload, seed: seed);
